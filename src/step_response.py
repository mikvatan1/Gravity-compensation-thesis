...
while (time.time() - start_time) < duration:
    try:
        ...
        if line.count(",") == 8:  # Expecting 9 values 
...
FORCE_TO_TARGET = 1.0 / (1.97 * 2)  # 1/(k_spring * num_springs)
ADC_TO_LOAD = (5.0 / 1023.0) * 29.361  # From main.cpp

# Calculate load for first 5s and last 5s
first_5s_data = df[df["time"] <= 5.0]
last_5s_data = df[df["time"] >= 5.0]

# Calculate loads in kg
first_5s_load_kg = (first_5s_data["detectedLoad"].mean() / 9.81) if len(first_5s_data) > 0 else 0.0
last_5s_load_kg = (last_5s_data["detectedLoad"].mean() / 9.81) if len(last_5s_data) > 0 else 0.0

# Calculate current load from detectedLoad directly
current_load = df["detectedLoad"].mean() if len(df) > 0 else 0.0
print(f"Detected Load: {current_load:.2f} N ({current_load/9.81:.1f} kg)")
print(f"First 5s Load: {first_5s_load_kg:.1f} kg")
print(f"Last 5s Load: {last_5s_load_kg:.1f} kg")

# Apply smoothing for lines
...
ax1.set_ylabel("Values (mm and control units)")
ax1.set_title(f"System Response\nFirst 5s: {first_5s_load_kg:.1f} kg, Last 5s: {last_5s_load_kg:.1f} kg")
ax1.legend()
ax1.grid(True, alpha=0.3)

# PID Contributions 
ax2.plot(df["time"], p_smooth, label="P ", linewidth=2.5, color='green', alpha=0.8)
ax2.plot(df["time"], i_smooth, label="I ", linewidth=2.5, color='blue', alpha=0.8)
ax2.plot(df["time"], d_smooth, label="D ", linewidth=2.5, color='purple', alpha=0.8)
ax2.axhline(y=0, color='black', linestyle=':', alpha=0.5)
ax2.set_xlabel("Time (s)")
ax2.set_ylabel("PID Contribution Value")
ax2.set_title(f"PID Contributions\nFirst 5s: {first_5s_load_kg:.1f} kg, Last 5s: {last_5s_load_kg:.1f} kg")
ax2.legend()
ax2.grid(True, alpha=0.3)

plt.tight_layout()

# Create PID_graphs folder
graphs_folder = "PID_graphs"
if not os.path.exists(graphs_folder):
    os.makedirs(graphs_folder)
    print(f"Created folder: {graphs_folder}")

# Generate filename with timestamp and average load info
timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
avg_load_kg = current_load / 9.81
filename = f"PID_analysis_{timestamp}_Load_{avg_load_kg:.1f}kg.png"
...
plt.plot(df["time"], p_smooth, label="P ", linewidth=2.0, color='green')
plt.plot(df["time"], i_smooth, label="I ", linewidth=2.0, color='blue')
plt.plot(df["time"], d_smooth, label="D ", linewidth=2.0, color='purple')
plt.axhline(y=0, color='black', linestyle=':', alpha=0.5)
plt.xlabel("Time (s)")
plt.ylabel("PID Contribution Value")
plt.title(f"PID Contributions Over Time\nFirst load: {first_5s_load_kg:.1f} kg, Last load: {last_5s_load_kg:.1f} kg")
plt.legend()
